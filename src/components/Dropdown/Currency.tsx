...
import { usdtAddy, busdAddy, usdcAddy } from 'src/config/test';
    ...
    <CurrencyDropDownWrapper data-aria-expanded={isOpen} ref={currencyRef}>
      <CurrencyDropDownButton onClick={() => setOpen(true)}>
        <DropdownLabel>
          ...
          <DropDownIcon>
            {isOpen ? (
              <RiArrowUpSFill style={{ width: '100%', height: '100%' }} />
            ) : (
              <RiArrowDownSFill style={{ width: '100%', height: '100%' }} />
            )}
          </DropDownIcon>
        </DropdownLabel>
